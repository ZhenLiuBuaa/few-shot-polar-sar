...
# parse gpu
parser.add_argument('--gpu', type=str, default=2, metavar='GPU',
                    help="gpus, default:0")
...
def main():
    ...
    for ep in range(config.epoch):
        ...
        metric_model.train()
        print(len(dataset))
        for sub_image, sub_label, support_mask in tqdm(data_loader):
            if torch.cuda.is_available():
                sub_image = sub_image.cuda(0)
                sub_label = sub_label.cuda(0)
                support_mask = support_mask.cuda(0)
            feature = sar_model(sub_image)
            loss, train_acc_dict_batch, test_acc_dict_batch = metric_model(feature, sub_label, support_mask)
            for acc_dict_one in train_acc_dict_batch:
            ...
            for acc_dict_one in test_acc_dict_batch:
                acc_matrix = np.array([list(acc_dict_one[key]) for key in acc_dict_one])
                sample_sum_each_class = acc_matrix.sum(0)
                for i, key in enumerate(acc_dict_one.keys()):
                    acc_dict[key.item()].update(acc_matrix[i][i], sample_sum_each_class[i])
        ...
        print("epoch: ", ep, "Base class:", base_acc, "   Novel class:", novel_acc)
        # print("Acc :", acc_dict)
        print("loss:", epoch_loss)
        print('base class:{}, novel class: {}'.format(dataset.base_class, dataset.novel_class))
