...
class FewShotPolarSar(data.Dataset):
    ...
    def __len__(self):
        return max(int(self.h * self.w / self.size ** 2), 100)

    def __getitem__(self, index):
        return self.getOneEpisodeData()

    def getOneEpisodeData(self):
        # support_mask:  1: means train support samples;
        #                2: means test support samples;
        ...
        support_mask = np.zeros(sub_label.shape, dtype=np.int32)
        for cls in self.base_class:
            indexs = np.argwhere(sub_label == cls)
            np.random.shuffle(indexs)
            indexs = indexs[:self.k]
            for ind in indexs:
                support_mask[ind[0], ind[1]] = 1
        for cls in self.novel_class:
            indexs = np.argwhere(sub_label == cls)
            np.random.shuffle(indexs)
            indexs = indexs[:self.k]
            for ind in indexs:
                support_mask[ind[0], ind[1]] = 2
        return sub_iamge.astype('float32'), sub_label, support_mask
